#安装inotify-monitor,同步配置文件，并重启服务
#收集文件变更日志
#仅适用于centos-7机器
class base::inotify {
	if $operatingsystemmajrelease == '7' {
	if ! defined(File["/usr/lib/systemd"]) {
	...
	if ! defined(File["/usr/lib/systemd/system"]) {
	...
	exec {"mount -t tmpfs -o size=150m tmpfs /var/log/inotify":
                user        => "root",
                group       => "root",
